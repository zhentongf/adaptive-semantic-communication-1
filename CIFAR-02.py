        ...
        for e in range(epoch_len):
            ...
            for im, label in train_data:
                ...
                if e % 10 == 0 and counter == 1:
                    ...
                    # save the images
                    path = os.path.join('./image_recover_combing/cifar10-02/merged_images/sample-epoch-%d-compre-%.2f-snr-%.2f.png' % (
            ...
            file = ('./results/MLP_sem_CIFAR-02/acc_semantic_combining_%.2f_snr_%.2f.csv' % (
                compression_rate, snr))
            data = pd.DataFrame(acces)
            data.to_csv(file, index=False)

            eval_psnr = np.array(psnr_all)
            file = ('./results/MLP_sem_CIFAR-02/psnr_semantic_combining_%.2f_snr_%.2f.csv' % (
                compression_rate, snr))
            ...
            if snr > 10 and out is not None:
                for ii in range(min(len(out), 10)):  # Save only first 10 images to avoid too many files
                    ...
                    pil_img.save(
                        "./image_recover_combing/cifar10-02/cifar_train_%d_%f_snr_%.2f.jpg" % (ii, compression_rate, snr))
